...
export class ProcessManager {
    ...
    ensureProcess({key, file, args, options}) {
        ...
        console.log(`msg="Started ${p.spawnargs.join(" ")}" service.type=${this.#serviceType}`);
        p.stdout.pipe(process.stdout);
        p.stderr.pipe(process.stderr);
        p.on("exit", async(code) => {
            console.info(`msg="Exiting ${p.spawnargs.join(" ")}" process.exit_code=${code} service.type=${this.#serviceType}`);
            this.#processMap.delete(key);
        });
        p.on("error", (err) => {
            console.error(`msg="Exiting ${p.spawnargs.join(" ")}" error.message=${err.message} service.type=${this.#serviceType}`);
            process.exit(1);
        });
